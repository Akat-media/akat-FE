...
export const updateAdAccountStatus = async (id: string, status: AdAccountStatus): Promise<void> => {
...
export const updateAdAccountMetrics = async (
  ...
  const { error } = await supabase.from('ad_accounts').update({ metrics }).eq('id', id);
